...
def runTest():
    ...
    try:
        ...
        if not is_json(output):
            if len(output) > 0:
                logging.error(
                    "CFSpeedtest CLI Error occurred that"
                    + "was not in JSON format"
                )
            return (0, 0, 0, 0, 0, 0, 0, 0, 0)
    except subprocess.TimeoutExpired:
        logging.error(
            "CFSpeedtest CLI process took too long to complete "
            + "and was killed."
        )
        return (0, 0, 0, 0, 0, 0, 0, 0, 0)
...
def updateResults():
    ...
    if datetime.datetime.now() > cache_until:
        ...
        server.info(
            {
                "server_location_city": str(r_server_city),
                "server_location_region": str(r_server_region),
            }
        )
        jitter.set(r_jitter)
        ping.set(r_ping)
...
if __name__ == "__main__":
    ...
    logging.info(
        "Starting Cloudflare-Speedtest-Exporter on http://localhost:"
        + str(PORT)
    )
